...
Validado contra exports reales de dos instancias distintas: comparten el
mismo layout de secciones letra por letra (mismo software SIGEVA, distinta
marca institucional), así que un solo parser sirve para todas — sólo cambia
`instancia_origen`, que se pasa como parámetro.
...
- La separación autor/título/revista se resuelve con heurísticas de texto
  (ver `_particionar_por_ultimo_limite`), no con un parser gramatical
  completo. Funciona bien en la práctica (validado con ~35 registros
  reales) pero un título que contenga ". " seguido de mayúscula puede
  cortarse antes de lo debido (ver caso "VALVULOAORTOPATÍA BICÚSPIDE." en
  el README de datos).
- Sólo se parsean las secciones de "antecedentes" en sentido estricto
  (PRODUCCION, SERVICIOS, trabajos en eventos no publicados). CARGOS,
  FINANCIAMIENTO CYT y FORMACION DE RRHH usan un layout de formulario
  multilínea muy distinto (etiqueta: valor repetido) y no se parsean
  todavía — queda para una siguiente etapa.
...
_RUIDO_RE = re.compile(
    r"^\s*(Currículum vitae|Curriculum vitae|UNIVERSIDAD|Secretaría|COMISION|MINISTERIO|"
    r"CONSEJO NACIONAL|PRODUCCION|Página \d+ de \d+)"
)
_FECHA_IMPRESION_RE = re.compile(r"Fecha de impresión:\s*\d{2}/\d{2}/\d{4}")
...
def _texto_completo(ruta: Path) -> str:
    ...
    for page in reader.pages:
        for linea in page.extract_text().split("\n"):
            if linea.strip() and not _RUIDO_RE.match(linea):
                lineas.append(linea.strip())
    texto = " ".join(lineas)
    texto = re.sub(r"\s+", " ", texto).strip()
    texto = _FECHA_IMPRESION_RE.sub("", texto)
    return texto
